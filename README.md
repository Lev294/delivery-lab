...
Учебный backend: **создать заказ → зарезервировать товар → завершить заказ**.
Версия 0.2: один FastAPI-сервис, PostgreSQL, атомарное резервирование, защита создания
от повторов, Docker Compose и интеграционные тесты. Это локальный учебный стенд.
...
стенда**; это явный сброс учебной БД. Новый том начинает со 100 единиц `demo-item`.
...
Учебные `GET /hello/{name}` и `/delivery/estimate/{distance_km}` сохранены.
...
- [Маршрут интенсива](docs/roadmap.md)
...
Первый endpoint оценки доставки написал ученик. Код, тесты, конфигурации и документацию
интенсива пишет наставник — Codex. Ученик изучает архитектуру и проводит эксперименты.
Готовность реализации не означает подтверждённого понимания.

Начальная работа сохранена в Git и [README первого урока](docs/history/lesson-00-readme.md).
