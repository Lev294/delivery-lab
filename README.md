...
Демонстрационный backend на Python и FastAPI. Проект постепенно развивается в сервис доставки: создание заказов, резервирование остатков и обновления статусов.

Текущий этап — один HTTP-сервис с демонстрационным приветствием и расчётом времени доставки. Условная формула: 10 минут подготовки плюс 3 минуты на каждый километр.
...
**Последствия:** запуск и путь запроса просты для анализа. По мере появления заказов и хранения данных потребуется выделить бизнес-логику и доступ к данным из HTTP-обработчиков.
...
2026-09-15 проверен коммит `78ca965` на macOS с Apple M1:

- Создан отдельный локальный Git-клон с шестью файлами репозитория, без `.venv`.
- Выполнен `uv sync --locked` с пустым отдельным кешем: зависимости скачаны заново, создано новое окружение.
- Приложение запущено из клона; проверены приветствие, расчёт для 5, 0 и 12 км, HTTP 422 для `abc` и HTTP 404 для неизвестного маршрута.
- Все проверки пройдены; отслеживаемые файлы клона после запуска не изменились.

Использован уже установленный Python 3.13.7. Проверка подтверждает восстановление проекта из Git и lock-файла на этой платформе; установка Python на чистой ОС и другие платформы отдельно не проверялись.
